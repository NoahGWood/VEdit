...
#dpg.display.set_caption('MoviePy')

 #       with dpg2.texture_registry(show=False):
 #           dpg2.add_raw_texture(self.w, self.h, self.raw_data, format=dpg2.mvFormat_Float_rgb, tag="video_tag")
def imdisplay(imarray, screen=None):
    """Splashes the given image array on the given pygame screen """
    h2, w2, d2 = imarray.shape
    raw_data = np.zeros((h2,w2,d2), dtype=np.float32)
    raw_data[:h2, :w2] = imarray[:,:]/255
    dpg.set_value("video_tag", raw_data)
#    a = dpg.surfarray.make_surface(imarray.swapaxes(0, 1))
#    if screen is None:
#        screen = dpg.display.set_mode(imarray.shape[:2][::-1])
#    screen.blit(a, (0, 0))
#    dpg.display.flip()
...
def preview(clip, fps=15, audio=True, audio_fps=22050, audio_buffersize=3000,
            audio_nbytes=2, fullscreen=False, pause_hook=None, ret_val=None):
    """ 
    ...
    """
    if fullscreen:
        flags = dpg.FULLSCREEN
    else:
        flags = 0
    
    # compute and splash the first image
    screen = None#dpg.display.set_mode(clip.size, flags)
